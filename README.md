...
gower_distance(X_num, X_cat, num_ranges)
	...
	•	输入：
	•	X_num: NumPy数组，形状为 (n, p)，包含 n 个样本的 p 个数值特征。如果没有数值特征则传入 None。
	•	X_cat: NumPy数组，形状为 (n, q)，包含 n 个样本的 q 个分类特征（类型为 object 或字符串）。如果没有分类特征则传入 None。
	•	num_ranges: NumPy数组，长度为 p，对应每个数值特征的取值范围（max-min）。用于归一化数值距离。如果某特征范围为0（所有值相等），函数内部会将其视为1以避免除零。
	...
	•	Input:
	•	X_num: a NumPy array of shape (n, p) containing p numeric features for n samples. If there are no numeric features, this can be None.
	•	X_cat: a NumPy array of shape (n, q) containing q categorical features for the same n samples (with dtype object or string). Use None if no categorical features.
	•	num_ranges: a NumPy array of length p providing the range (max minus min) for each numeric feature, used to normalize numeric differences. If a feature’s range is 0 (all values identical), the code treats it as 1 to avoid division by zero.
...
classical_mds_from_distance(D, max_points, seed=42)
	...
	•	输入：
	•	D: NumPy二维数组，形状为 (n, n)，输入的距离矩阵。
	•	max_points: 整数，指定参与MDS降维的最大样本数。如果 n > max_points，则函数将随机选择 max_points 个样本进行MDS；如果 n 较小则使用全部样本。
	•	seed: 整数，随机种子，用于在 n 很大时重复选择子样本，使每次运行结果可重复（默认为42）。
	...
	•	Input:
	•	D: a NumPy 2D array of shape (n, n) representing the input distance matrix for n samples.
	•	max_points: an integer specifying the maximum number of points to include in MDS. If n > max_points, the function will randomly choose max_points samples for the MDS computation; if n is small enough, all samples are used.
...
run_kprototypes(df_prep, numeric_cols, categorical_cols)
	...
	•	best_k: 整数，成本最小的最佳簇数 K。
	•	best_labels: NumPy数组，长度为样本数，每个元素是对应样本的簇标签（0 ~ K-1），来自最佳模型。
	•	best_model: 训练后的 KPrototypes 模型对象（kmodes.KPrototypes 类型），对应最佳 K。
	•	cost_records: 字典，各候选 K 值对应的成本，如 {6: 1234.5, 7: 1100.8, ...}。
	...
	•	best_k: an integer representing the number of clusters K that yielded the lowest cost.
	...
	•	cost_records: a dictionary mapping each tried K to its cost (e.g., {6: 1234.5, 7: 1100.8, ...}).
